...
warp.onmouseup=function(e) {
    ...
    if (cutting&&!space) {
        show_def_window(true,function() {
            ...
            document.querySelectorAll('.menu>a')[7].click();//重置画布位置
        },"确定要裁剪吗?目前不可逆的哦!");//裁剪
...
//取消选择
document.querySelectorAll('.menu>a')[2].onclick = function () {
    select_null();
}
//设置容差
...
//重置画布位置
document.querySelectorAll('.menu>a')[7].onclick=function() {
    c.style.left="0px";
    c.style.top="0px";
}
//删除选区
document.querySelectorAll('.menu>a')[3].onclick=function() {
    ps.delet_s();
    select_null();
}
//开启 涂鸦
document.querySelectorAll('.menu>a')[8].onclick=function() {
    if (pen) {
        pen=false;
...
//开启 裁剪
document.querySelectorAll('.menu>a')[5].onclick=function() {
    if (cutting) {
        cutting=false;
        this.style.backgroundColor=null;
        cut_box.style.display="none";
...
//开启 橡皮擦
document.querySelectorAll('.menu>a')[9].onclick=function() {
    if (clear_pen) {
        clear_pen=false;
        this.style.backgroundColor=null;
    }else if(!pen&&!cutting){
...
document.querySelectorAll('.menu>a')[4].onclick=function() {
...
document.querySelector(".color>h4>span").onclick=function () {
    document.querySelectorAll('.menu>a')[4].click();
}
...
document.querySelectorAll('.menu>a')[1].onclick=function() {
...
// 通用窗口-----e
// 查看 帮助
document.querySelectorAll(".menu>a")[10].onclick=function () {
    show_def_window(true,function() {show_def_window(false)},"无视打开图形，可以直接打开图片文件使用。双击进入抠图选区,支持裁剪,简单调色,按住空格,鼠标可以拖到画布.可导出多种格式,简单方便!","使用手册");
}